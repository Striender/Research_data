...
RESULT_ROOT=results_4core/Bingo/
...
shopt -s nullglob
AI_ML_TRACES=(
    "$TRACE_DIR"/*.champsimtrace.gz
    "$TRACE_DIR"/*.champsimtrace.xz
)
shopt -u nullglob

for TRACE in "${AI_ML_TRACES[@]}"; do
...
if [ "$TRACE_COUNT" -eq 0 ]; then
    echo "❌ No AI/ML traces found in $TRACE_DIR"
    exit 1
fi
...
run_traces() {
    ...
    for TRACE in "${AI_ML_TRACES[@]}"; do
        ...
        NAME="${NAME%.champsimtrace.xz}"
        echo "$BINARY \
          -warmup_instructions $WARMUP \
          -simulation_instructions $SIM \
          -traces \"$TRACE\" \"$TRACE\" \"$TRACE\" \"$TRACE\" \
          > \"$OUT_DIR/$NAME\"" >> "$TMP"
    done

    JOBS=$(wc -l < "$TMP")

    TRACE_PARALLEL=$TOTAL_CORES
    [ "$TRACE_PARALLEL" -lt 1 ] && TRACE_PARALLEL=1
    [ "$TRACE_PARALLEL" -gt "$JOBS" ] && TRACE_PARALLEL=$JOBS

    if [[ "$RP_MODE" == "p" ]]; then

        # divide available cores across parallel replacement-policy runs
    ...
    echo "🚀 Launching $JOBS 4-core simulations using $TRACE_PARALLEL parallel jobs"
...
run_single_rp() {
    ...
    binary="$BIN_DIR/hashed_perceptron-no-${L1_BIN}-${L2_BIN}-no-no-no-no-lru-lru-${l1d_repl}-${base}-${pol}-lru-lru-lru-4core-no"
