...
constexpr uint64_t REGION_SIZE =  2 * 1024; // '4KB', '8KB', '16KB, '32KB', '64KB, '128KB', '512KB', '1024KB', '2048KB'    
// Gaze uses 4 KiB regions, so log2(REGION_SIZE) is fixed at 12. The old
// ChampSim tree does not provide champsim::lg2.
constexpr uint64_t LOG2_REGION_SIZE = 11;  /// -----------------------------> need to update this value if REGION_SIZE is changed
...
struct AccumulateTableData {
    ...
    AccumulateTableData(uint64_t trigger_offset_, uint64_t second_offset_,
                        uint64_t pc_, bool missed_in_pt_,
                        const std::vector<bool>& pattern_,
                        const std::vector<int>& order_, int last_stride_,
                        uint64_t last_offset_, bool con_)
        : trigger_offset(trigger_offset_), second_offset(second_offset_), pc(pc_),
          missed_in_pt(missed_in_pt_), pattern(pattern_), order(order_),
          last_stride(last_stride_), last_offset(last_offset_), con(con_) {}
};
