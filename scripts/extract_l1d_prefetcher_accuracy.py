...
DEFAULT_RESULTS_DIR = os.path.normpath(
    os.path.join(SCRIPT_DIR, "..", "results", "bingo_region_L1D")
)
DEFAULT_OUTPUT_FILE = "../Excel_Output/bingo_region_L1D/bingo_l1d_prefetcher_accuracy.xlsx"
...
def parse_file(filepath):
    """
    Parses a single ChampSim output file to extract L1D prefetch statistics.
    ...
    prefetch_pattern = (
        r"L1D\s*PREFETCH\s*REQUESTED:\s*\d+\s*"
        r"ISSUED:\s*\d+\s*"
        r"USEFUL:\s*(\d+)\s*"
        r"USELESS:\s*(\d+)"
    )
    ...
    late_pattern = (
        r"L1D\s*TIMELY\s*PREFETCHES:\s*\d+\s*"
    ...
    useful_load = int(acc_match.group(1))
    issued_lower = int(acc_match.group(2))
    ...
    return useful_load, issued_lower, accuracy, useless, late
...
def collect_records(results_dir):
    ...
    for root, _, files in os.walk(results_dir):
        for filename in sorted(files, key=natural_sort_key):
            ...
            useful, issued, accuracy, useless, late = stats
...
def write_workbook(records, output_file):
    ...
    for configuration in sorted(grouped_records, key=natural_sort_key):
        ...
        for record in sorted(
            ...
            worksheet.cell(row_idx, 5, record["useless"])
            worksheet.cell(row_idx, 6, record["late"])
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract L1D prefetch statistics from ChampSim output files."
    )
