...
def parse_file(filepath):
    """
    Parses a single ChampSim output file to find the L2C prefetcher accuracy stats.
    Example line to match:
    L2C USEFUL LOAD PREFETCHES:    6733170 PREFETCH ISSUED TO LOWER LEVEL:    7342702  ACCURACY: 91.6988
    """
    ...
    pattern = (
        r"L2C\s*USEFUL\s*LOAD\s*PREFETCHES:\s*(\d+)\s*"
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract L2C prefetcher accuracy statistics from ChampSim output files."
    )
    parser.add_argument(
        "results_dir",
        nargs="?",
    ...
    if not records:
        raise SystemExit(
            ...
            f"{args.results_dir}\n"
            "Expected lines containing 'L2C USEFUL LOAD PREFETCHES:'."
        )
